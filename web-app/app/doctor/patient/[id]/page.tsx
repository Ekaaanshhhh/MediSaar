...
import { ArrowLeft, FileText, ActivitySquare, Pill, Sparkles, TriangleAlert } from 'lucide-react';
...
export default function PatientMedicalSummaryPage() {
  ...
  const patientAge = new Date().getFullYear() - new Date(profile.dateOfBirth).getFullYear();
        ...
        <div className="w-full max-w-2xl bg-surface border-l-4 border-l-amber-500 rounded-lg p-8 md:p-10 shadow-lift relative">
          ...
          <div className="mb-8">
            ...
            <p className="text-sm font-medium text-ink-500 mt-1 font-sans">
              {patientAge} years old {profile.bloodGroup ? `· Blood Group ${profile.bloodGroup}` : ""} · ID: {profile.medisaarId}
            </p>
          </div>
          ...
          <div className="space-y-6">
            ...
            <section className="border border-border/50 rounded-md p-5 bg-canvas">
              ...
              {prescriptions.length > 0 ? (
                <ul className="space-y-2">
                  {prescriptions.flatMap(p => p.medications).map((med, idx) => (
            ...
            <section className="border border-border/50 rounded-md p-5 bg-canvas">
              ...
              {profile.currentConditions.length > 0 ? (
                <ul className="space-y-2">
                  {profile.currentConditions.map((cond) => (
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-ink-500 mt-1 font-sans">
              {patientAge} years old · Blood Group {profile.bloodGroup} · ID: {profile.medisaarId} · Phone: {profile.phone}
            </p>
          </div>
        </div>
      </div>
            ...
            <TabsContent value="reports">
              {reports.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {reports.map(r => (
                    <Card key={r.id} className="cursor-pointer border-border/60 hover:border-sage-600 bg-surface shadow-soft transition-all duration-[220ms]">
                      <CardHeader className="p-4 pb-2">
                        <div className="flex justify-between items-start">
                          ...
                          <span className="text-xs text-ink-300 font-sans">{format(new Date(r.date), 'MMM dd, yyyy')}</span>
                        </div>
            ...
            <TabsContent value="prescriptions">
               {prescriptions.length > 0 ? (
                 <div className="space-y-4">
                   {prescriptions.map(p => (
                     <Card key={p.id} className="bg-surface border-border/60 shadow-soft">
               ...
               {visits.filter(v => v.diagnosis).length > 0 ? (
                 <div className="space-y-4">
                   {visits.filter(v => v.diagnosis).map(v => (
                     <Card key={v.id} className="bg-surface border-border/60 shadow-soft">
                       <CardHeader className="p-4 pb-2">
                         <span className="text-xs text-ink-300 font-sans">{format(new Date(v.date), 'MMM dd, yyyy')}</span>
